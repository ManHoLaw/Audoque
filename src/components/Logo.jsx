...
const Logo = ({ setSelection }) => {
  ...
  return (
    <h1 class='w-full flex pb-5 justify-evenly'>
        <div class='flex items-baseline max-sm:justify-start max-sm:flex-none max-sm:hidden pt-15'>
            <button class='cursor-pointer rounded-3xl' onClick={handleLogoClick}>
                <img src={bannerImg} alt='Au_banner' width={300}/>
            </button>
        </div>
        <div class='flex items-baseline max-sm:justify-center sm:hidden max-sm:flex-2 pt-10'>
            <button class='cursor-pointer rounded-3xl' onClick={handleLogoClick}>
